...
function respawnAtCheckpoint() {
  const cp = checkpoints[checkpointIndex>=0 ? checkpointIndex : 0];
...
function update(dtMs){
  ...
  if(keys["w"]||keys["arrowup"]){ spaceship.vx += Math.cos(spaceship.angle)*thrustAccel*s; spaceship.vy += Math.sin(spaceship.angle)*thrustAccel*s; }
  if(keys["s"]||keys["arrowdown"]){ spaceship.vx *= brakeFactor; spaceship.vy *= brakeFactor; }
  ...
  for(let i=particles.length-1;i>=0;i--){
    ...
    const ss=dtMs/1000; p.x+=p.vx*ss; p.y+=p.vy*ss; p.vx*=0.96; p.vy*=0.96;
  }
...
function draw(){
  ...
  // planets
  for(const p of planets){
    const grad = ctx.createRadialGradient(p.x,p.y,p.size*0.5,p.x,p.y,p.size*1.8);
  ...
  // spaceship
  ctx.save(); ctx.translate(spaceship.x, spaceship.y); ctx.rotate(spaceship.angle);
  if(keys["w"]||keys["arrowup"]){
    const flameLength=8+Math.random()*4;
    ctx.fillStyle="#ffb86c";
    ctx.beginPath(); ctx.moveTo(-spaceship.radius*0.9,0); ctx.lineTo(-spaceship.radius*0.9-flameLength,4); ctx.lineTo(-spaceship.radius*0.9-flameLength,-4); ctx.closePath(); ctx.fill();
  }
